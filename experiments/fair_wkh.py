...
# --- ADMM weight refinement (fairness-aware) for subset ---------------------
class FairADMMSubset:
    """
    ADMM refinement on selected subset S.
    Objective: minimize w^T K w - 2 k^T w
    With ADMM we add a fairness penalty via z-update (approximate PGD projection used previously).
    This is a direct adaptation of your earlier ADMM class restricted to S.
    """
    def __init__(self, K_SS_torch, k_S_torch, sensitive_mask_sub, outcome_mask_sub,
                 rho=1.0, lr_fair=0.1, projection_steps=20, device=DEVICE):
        self.K = K_SS_torch.to(device)
        self.k = k_S_torch.to(device)
        self.N = self.K.shape[0]
        self.rho = rho
        self.lr_fair = lr_fair
        self.projection_steps = projection_steps
        self.device = device

        # Precompute inverse (K + (rho/2) I)^{-1}
        I = torch.eye(self.N, device=self.device)
        self.inv = torch.inverse(self.K + (self.rho / 2.0) * I)

        # masks (torch bool)
        self.male_mask = sensitive_mask_sub.to(self.device)
        self.female_mask = ~self.male_mask
        self.male_outcome_mask = self.male_mask & outcome_mask_sub.to(self.device)
        self.female_outcome_mask = self.female_mask & outcome_mask_sub.to(self.device)

    def _fairness_violation(self, z):
        eps = 1e-8
        p_male = z[self.male_outcome_mask].sum() / (z[self.male_mask].sum() + eps)
        p_female = z[self.female_outcome_mask].sum() / (z[self.female_mask].sum() + eps)
    ...
    def solve(self, iterations=200):
        ...
        for i in range(iterations):
            ...
            w = self.inv @ rhs
            # z update: gradient-step on fairness loss (squared) starting from v = w + u
            v = w + u
            z_var = v.clone().detach().requires_grad_(True)
            for _ in range(self.projection_steps):
                loss = self._fairness_violation(z_var) ** 2
                grad, = torch.autograd.grad(loss, z_var, retain_graph=False)
                z_var = z_var - self.lr_fair * grad
            # project z_var onto nonneg simplex (approx): clamp negative then normalize
            z_tmp = z_var.detach()
            z_tmp = torch.clamp(z_tmp, min=0.0)
            s = z_tmp.sum()
            if s > 0:
                z = z_tmp / s
            else:
                z = torch.ones_like(z_tmp) / self.N
            u = u + w - z
        return w.detach()
...
def baseline_fair_admm(P_tensor, sensitive_idx, outcome_idx, sigma, m, rho=1.0, lr_fair=0.1, proj_steps=20, ridge=1e-8, fairness_beta=0.0, verbose=False):
    """
    Fair baseline pipeline:
     - fairness-aware greedy selection (small fairness_beta used in selection)
    ...
    S, _, K = weighted_kernel_herding_frank_wolfe_alternate(
        P_np, m, sigma=sigma, ridge=ridge, verbose=verbose,
        fairness_beta=fairness_beta, sensitive_mask=sensitive_idx, outcome_mask=outcome_idx, alternate_dp_freq=None)

    # prepare subset tensors and masks
    P_S = P_tensor[S, :].clone()
    K_SS_torch = rbf_kernel_torch(P_S, P_S, sigma=sigma)
    # compute k_S (target mean) as torch
    K_full_torch = rbf_kernel_torch(P_tensor, P_S, sigma=sigma)
    ...
    sens_sub = torch.from_numpy(sens_np[S]).to(DEVICE)
    out_sub = torch.from_numpy(out_np[S]).to(DEVICE)

    # ADMM refine on subset
    admm = FairADMMSubset(K_SS_torch, k_S_torch, sens_sub, out_sub, rho=rho, lr_fair=lr_fair, projection_steps=proj_steps, device=DEVICE)
    w_sub_torch = admm.solve(iterations=200)

    # ensure nonneg and sum-to-one (admm may produce small negatives) - enforce by projection if needed
...
# --- UPDATED baseline_fair_admm: accept dp_frequency ------------------------
def baseline_fair_admm(
    P_tensor, sensitive_idx, outcome_idx, sigma, m,
    rho=1.0, lr_fair=0.1, proj_steps=20,
    ridge=1e-8, fairness_beta=0.0, dp_frequency=None, verbose=False
):
    """
    Fair baseline pipeline:
    ...
    # ADMM refine on subset
    admm = FairADMMSubset(K_SS_torch, k_S_torch, sens_sub, out_sub,
                          rho=rho, lr_fair=lr_fair, projection_steps=proj_steps, device=DEVICE)
    w_sub_torch = admm.solve(iterations=200)

    # ensure nonneg and sum-to-one (ADMM may produce small negatives)
    w_sub = w_sub_torch.cpu().numpy()
    w_sub = np.maximum(w_sub, 0.0)
    s = w_sub.sum()
    if s <= 0:
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    w_alt_admm, alt_admm_indices = baseline_fair_admm(
        P, sensitive_idx, outcome_idx, sigma=SIGMA, m=CORESET_M,
        rho=RHO, lr_fair=0.05, proj_steps=25, ridge=1e-8,
        fairness_beta=0.0, dp_frequency=DP_FREQ, verbose=True
    )
    timings['Alternate + ADMM'] = time.time() - t0
    results['Alternate + ADMM'] = (w_alt_admm, alt_admm_indices)

    # Evaluate all baselines
    names = ['Random', 'Standard WKH', 'Simple Alternate', 'Alternate + QP', 'Alternate + ADMM']
    stats_list = []
