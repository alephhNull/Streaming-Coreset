...
class StreamingCoresetWeightTransfer(AbstractStreamingCoreset):
    def __init__(
        self,
        M: int,
        D: int,
    ...
    def _evict_with_weight_transfer(self):
        """
        Evicts the point with the lowest weight from the buffer.
        Transfers its weight to the nearest point in the buffer, 
        regardless of class label.
        """
        evict_idx = int(np.argmin(self.buffer_weights))
        evicted_weight = self.buffer_weights[evict_idx]
        target_z = self.buffer_Z[evict_idx]

        # Calculate Euclidean distances to all points in the buffer
        distances = np.linalg.norm(self.buffer_Z - target_z, axis=1)
        
        # Set the distance to itself to infinity so it isn't chosen as its own neighbor
        distances[evict_idx] = np.inf
        
        # Find the absolute nearest neighbor
        closest_idx = int(np.argmin(distances))

        # Transfer the weight
        self.buffer_weights[closest_idx] += evicted_weight

        # Remove the evicted point
        ...
        # Re-normalize weights to ensure stability
    ...
    def _process_point(self, x_raw, y_label, z_rff, batch_idx, local_idx):
        ...
        # Use the weight transfer eviction method
        if len(self.buffer_Z) > self.M:
            self._evict_with_weight_transfer()
