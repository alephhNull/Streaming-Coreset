...
from streamers.seperation_gaurded_streamer import (
...
def estimate_drift_bound(X: np.ndarray, sampler: RBFSampler) -> float:
    """
...
def main():
    ...
    sampler.fit(X)
    print(f"    RFF dim D={RFF_DIM}, gamma={RFF_GAMMA}")
    ...
    streamer = SeparationGuardedFrankWolfeStreamer(M=M, D=RFF_DIM, delta_drift_max=delta_drift_safe, nu_separation=NU_SEPERATION, sampler=sampler, batch_size=BATCH_SIZE, verbose=False, hyperparams=hp)
